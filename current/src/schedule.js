...
function populateSchedule() {
    ...
    const saturdayEvents = [
        ...
        },
        {
            ...
            location: "",
            notes: "Meet new people, find a team, and start brainstorming projects at this workshop!",
            category: "event",
        ...
        // },
        {
            ...
            event: "Lunch",
            location: "",
            notes: "Wait until your group is called, and have your hacker ID out to scan!",
            category: "food",
        ...
        // },
        {
            ...
            event: "Cup Stacking",
            location: "",
            notes: "A hackathon tradition! Find a team and stack the most cups for a prize!",
            category: "fun",
            image: "assets/img/ns_pink.png"
        },
        {
    ...
    const sundayEvents = [
        ...
        },
        {
            ...
            location: "",
            notes: "Show up early and set up your project before judges start going around!",
            category: "required",
            image: "assets/img/ns_blue.png"
        },
        {
        ...
        },
        {
            ...
            location: "",
            notes: "If you're pitching, be ready - this is a rapid fire round! 30 seconds to a minute per person",
            category: "event",
            image: "assets/img/ns_green.png"
        },
        {
...
export function filterEvents(category) {
    ...
    allEvents.forEach((event) => {
        ...
        if (category === "all" || event.dataset.category === category) {
            ...
            image.classList.remove("shadow-blue", "shadow-green", "shadow-yellow", "shadow-pink");
    ...
    allFilters.forEach((filter) => {
        if (filter.id === category) {
            filter.style.color = getComputedStyle(document.body).getPropertyValue("--white");
            filter.classList.remove("schedule-button-hover");
            switch (category) {
                case "all":
                    filter.style.background = getComputedStyle(document.body).getPropertyValue("--white");
                    filter.style.color = getComputedStyle(document.body).getPropertyValue("--black");
                    break;
                case "required":
                    filter.style.background = getComputedStyle(document.body).getPropertyValue("--blue");
                    break;
                case "event":
                    filter.style.background = getComputedStyle(document.body).getPropertyValue("--green");
                    break;
                case "food":
                    filter.style.background = getComputedStyle(document.body).getPropertyValue("--yellow");
                    break;
                case "fun":
                    filter.style.background = getComputedStyle(document.body).getPropertyValue("--pink");
                    break;
            }
        } else {
            ...
            switch (filter.id) {
                case "all":
                    ...
                    break;
                case "required":
                    filter.style.color = getComputedStyle(document.body).getPropertyValue("--blue");
                    break;
                case "event":
                    filter.style.color = getComputedStyle(document.body).getPropertyValue("--green");
                    break;
                case "food":
                    filter.style.color = getComputedStyle(document.body).getPropertyValue("--yellow");
                    break;
                case "fun":
                    filter.style.color = getComputedStyle(document.body).getPropertyValue("--pink");
                    break;
            }
        }
    })
