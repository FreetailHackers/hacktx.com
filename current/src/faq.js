...
export function togglefaq(q) {
  ...
  if (dot.classList.contains("faq-dot-active")) dot.classList.remove("faq-dot-active");
  else dot.classList.add("faq-dot-active");

  let question = document.getElementById(q.replaceAll(" ", "-") + "q");

  if (question.classList.contains("faq-glow")) question.classList.remove("faq-glow");
  else question.classList.add("faq-glow");

  let answer = document.getElementById(q.replaceAll(" ", "-") + "a");

  if (answer.style.maxHeight == "0px") answer.style.maxHeight = answer.scrollHeight + "px";
